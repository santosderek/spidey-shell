...
use crate::persistence::{Conversation, ConversationStore};

use super::models::{ConversationMode, InputMode, SearchMode, TitledMessage};
...
pub struct AppState {
    ...
    pub conv_id_to_index: HashMap<i64, usize>,
...
impl AppState {
    pub fn new(mcp_manager: &Option<MCPServerManager>, store: &ConversationStore) -> Self {
        let mut app = Self {
    ...
    /// Load conversations from the database
    pub fn load_conversations(&mut self, store: &ConversationStore) {
        // Clear current data
        ...
        self.conversations_list
            .push("+ New Conversation".to_string());
        self.conversation_modes.push(ConversationMode::New);
        self.conversations_list
            .push("All Conversations".to_string());
        ...
        if let Ok(conversations) = store.get_all_conversations() {
            // Sort conversations by created_at timestamp (newest first)
            let mut sorted_conversations = conversations;
            sorted_conversations.sort_by(|a, b| b.created_at.cmp(&a.created_at));
    ...
    pub fn get_conversation_id_by_index(&self, index: usize) -> Option<i64> {
    ...
    pub fn get_conversation_id_by_title(&self, title: &str) -> Option<i64> {
