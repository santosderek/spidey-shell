...
use super::models::{InputMode, SearchMode, ConversationMode};
...
fn render_conversations_list(f: &mut Frame, app: &AppState, area: Rect) {
    let items: Vec<ListItem> = app.conversations_list.iter().enumerate().map(|(index, s)| {
        ...
        let (display_text, item_style) = if index < 2 {
            // Special items
            match index {
                0 => (s.to_string(), Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD)),
