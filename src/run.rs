...
use crate::openai::{AzureOpenAIClient, AzureOpenAIConfig};
...
pub async fn run_app() -> Result<(), Box<dyn Error>> {
    ...
    // Initialize tracing
    if let Err(e) = config::init_tracing() {
        eprintln!("Warning: Failed to initialize tracing: {}", e);
    }
    
    info!("Starting spidey-shell application");
    ...
    if !has_tty {
        error!("Application requires a TTY terminal to run");
    ...
    info!("Initializing conversation store at: {}", db_path.display());
    let store = match ConversationStore::new(db_path) {
        Ok(store) => store,
        Err(e) => {
            error!("Failed to initialize conversation store: {}", e);
            return Err(Box::new(e));
        }
    };

    // Set up OpenAI client
    info!("Setting up Azure OpenAI client");
    let oaiclient = setup_openai_client(&credential_manager)?;

    // Set up MCP server manager
    info!("Setting up MCP server manager");
    let mut mcp_manager = setup_mcp_manager(&credential_manager)?;

    // Set up terminal
    info!("Setting up terminal interface");
    ...
    if let Err(e) = cleanup_terminal(terminal) {
...
fn setup_openai_client(
    ...
    info!("Retrieving Azure OpenAI credentials");
    ...
    if api_key.is_empty() || api_base.is_empty() || deployment_id.is_empty() {
        warn!("Missing required Azure OpenAI credentials");
        ...
        info!("Azure OpenAI configured with model: {}, deployment: {}", model, deployment_id);
    }
...
fn setup_mcp_manager(
    ...
    info!("Setting up MCP server base directory at: {}", mcp_base_dir.display());

    match MCPServerManager::new(mcp_base_dir.clone()) {
        Ok(mut manager) => {
            info!("MCP server manager initialized successfully");
            ...
            if server_names.is_empty() {
                ...
                info!("Discovered {} MCP server(s)", server_names.len());
                println!("Discovered {} MCP server(s):", server_names.len());
                for name in &server_names {
                    if let Some(config) = manager.get_server(name) {
                        info!("MCP server: {} ({})", name, config.description.as_deref().unwrap_or("No description"));
                        if !config.capabilities.is_empty() {
                            info!("Server capabilities: {}", config.capabilities.join(", "));
                        }
            ...
            info!("Starting all Python MCP processes");
            match manager.start_all_python_processes() {
                Ok(started) => {
                    if started > 0 {
                        info!("Started {} Python MCP process(es)", started);
                        println!("Started {} Python MCP process(es)", started);
                    } else {
                        info!("No Python MCP processes were started");
                    }
                }
                Err(e) => {
                    warn!("Failed to start some Python MCP processes: {}", e);
        ...
        Err(e) => {
            error!("Failed to initialize MCP server manager: {}", e);
...
fn setup_terminal() -> Result<Terminal<CrosstermBackend<io::Stdout>>, Box<dyn Error>> {
    info!("Enabling terminal raw mode");
    enable_raw_mode().map_err(|e| {
        error!("Failed to enable raw mode: {}", e);
        Box::<dyn Error>::from(format!("Failed to enable raw mode: {}", e))
    })?;
    ...
    execute!(stdout, EnterAlternateScreen, EnableMouseCapture)
        .map_err(|e| {
            ...
            Box::<dyn Error>::from(format!("Failed to setup terminal: {}", e))
        })?;

    info!("Creating terminal backend");
    let backend = CrosstermBackend::new(stdout);
    Terminal::new(backend).map_err(|e| {
        error!("Failed to create terminal: {}", e);
        Box::<dyn Error>::from(format!("Failed to create terminal: {}", e))
    })
}
...
fn cleanup_terminal(
    ...
    if let Err(e) = execute!(
        ...
        error!("Failed to execute terminal cleanup: {}", e);
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            format!("Failed to execute terminal cleanup: {}", e),
        )));
    }
