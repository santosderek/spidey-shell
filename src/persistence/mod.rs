...
impl ConversationStore {
    pub fn new(db_path: PathBuf) -> Result<Self> {
        ...
        let has_mcp_server = conn.query_row::<bool, _, _>(
    ...
    pub fn add_message_with_server(&self, conversation_id: i64, message: &Message, mcp_server: &str) -> Result<()> {
        let now = chrono::Utc::now().timestamp();
        self.conn.execute(
    ...
    pub fn get_conversation(&self, id: i64) -> Result<Option<Conversation>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, title, created_at, mcp_servers FROM conversation WHERE id = ?"
        )?;
