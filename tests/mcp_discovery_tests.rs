...
fn create_non_mcp_project(base_dir: &PathBuf, project_name: &str) -> PathBuf {
    ...
    file.write_all(b"print('Not an MCP project')").expect("Failed to write to main.py");
...
fn test_project_metadata_extraction() {
    ...
    let init_path = package_dir.join("__init__.py");
    let mut init_file = File::create(init_path).expect("Failed to create __init__.py");
    init_file.write_all(b"").expect("Failed to write to __init__.py");
...
"#,
    ...
    if project.metadata.description.is_some() {
        assert_eq!(project.metadata.description, Some("Test metadata extraction".to_string()));
    }
    
    // Check MCP capabilities - these are important for server functionality
    assert!(project.metadata.mcp_capabilities.contains(&"text-generation".to_string()));
    assert!(project.metadata.mcp_capabilities.contains(&"image-generation".to_string()));
    assert!(project.metadata.mcp_capabilities.contains(&"audio-transcription".to_string()));
}
...
fn test_project_creation() {
    ...
    let project_dir = discovery.create_project(project_name).expect("Failed to create project");
    ...
    assert!(project_dir.join("src").join(project_name).join("main.py").exists());
