...
fn test_config_from_env() {
    ...
    assert!(
        config.is_ok(),
        "Should be able to load config from environment"
    );
...
async fn test_api_endpoint_connectivity() {
    let config = match AzureOpenAIConfig::from_env() {
        ...
        Err(e) => {
            error!(
                "❌ Skipping connectivity test: missing Azure API config: {}",
                e
            );
            return;
        }
...
async fn test_send_chat_inference() {
    let config = match AzureOpenAIConfig::from_env() {
        ...
        Err(e) => {
            error!(
                "❌ Skipping inference test: missing Azure API config: {}",
                e
            );
            return;
        }
...
async fn test_send_chat_with_system_message() {
    let config = match AzureOpenAIConfig::from_env() {
        ...
        Err(e) => {
            error!(
                "❌ Skipping system message test: missing Azure API config: {}",
                e
            );
            return;
        }
