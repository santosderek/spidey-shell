...
fn test_basic_server_config_loading() {
    ...
    assert!(
        servers.contains(&server_name.to_string()),
        "Should load the test server"
    );
...
fn test_server_commands_parsing() {
    ...
    let (modified_input, servers) =
        manager.parse_server_commands("Ask @test_server to do something");
    ...
    let (modified_input, servers) =
        manager.parse_server_commands("Ask @test_server and @another_server to do something");
    ...
    let (modified_input, servers) =
        manager.parse_server_commands("Ask @invalid_server to do something");
...
fn test_create_python_project() {
    ...
    assert!(
        project_dir.join("src").join(project_name).exists(),
...
fn test_python_process_management() {
    ...
    if let Ok(started) = result {
        if started {
            ...
            assert!(
                stop_result.is_ok(),
                "Should be able to stop the Python process"
            );
        } else {
